...
{
    public class ProtectionmarginController : Controller
    {
        QLNhaThauEntities db_context = new QLNhaThauEntities();
        // GET: Protectionmargin
        public ActionResult Index(int? page, int? IDViTri)
        {
            ...
            var res = (from a in db_context.DinhBienBHLDs
                      join vt in db_context.Vitris on a.VTLVID equals vt.IDViTri
                       select new ProtectionmarginValidation()
                       {
                        IDDB = a.IDDB,
        ...
        [HttpPost]
        public ActionResult Create(ProtectionmarginValidation _DO)
        {
                try
                {

                    db_context.DinhBienBHLD_insert(_DO.TenVTBHLD, _DO.VTLVID);
            ...
            return RedirectToAction("Index", "Protectionmargin");
        }
        public ActionResult Delete(int id)
        {
            try
            {
                db_context.DinhBienBHLD_delete(id);
            }
            catch (Exception e)
            {
        ...
        {
            var res = (from a in db_context.DinhBienBHLD_searchByID(id)
                       select new ProtectionmarginValidation
                       {
                           IDDB = a.IDDB,
                           TenVTBHLD = a.TenVTBHLD,
            ...
            ProtectionmarginValidation DO = new ProtectionmarginValidation();
        ...
        [HttpPost]
        public ActionResult Edit(ProtectionmarginValidation _DO)
        {
            try
            {

                db_context.DinhBienBHLD_update(_DO.IDDB, _DO.TenVTBHLD, _DO.VTLVID);
        ...
        [HttpPost]
        public ActionResult ImportExcel(ProtectionmarginValidation _DO)
        {
                        ...
                        try
                        {
                            for (int i = 2; i < dt.Rows.Count; i++)
                            {
                                if (dt.Rows[i][0].ToString() != "")
                                {
                                    ...
                                    System.Data.Entity.Core.Objects.ObjectParameter returnIDDB = new System.Data.Entity.Core.Objects.ObjectParameter("IDDB", typeof(int));
                                    db_context.DinhBienBHLD_insertDS(BHLĐTVT, _DOVT.IDViTri, returnIDDB);
                                    int IDDB = Convert.ToInt32(returnIDDB.Value);
                                    ID = returnIDDB.Value.ToString();
                                    ...
                                    db_context.DinhBienChiTiet_insertDS(_DOL.IDBHLD, IDDB, ĐVT, Convert.ToInt32(S_SL), Convert.ToInt32(T_TH), GhiChu);
                                ...
                                else
                                {
                                    ...
                                    db_context.DinhBienChiTiet_insertDS(_DOL.IDBHLD, Convert.ToInt32(ID), ĐVT, Convert.ToInt32(S_SL), Convert.ToInt32(T_TH), GhiChu);
        ...
        public ActionResult DownExcelFile(object sender, EventArgs e)
        {
            ...
            string filePath = "C:\\Users\\HPDQ\\Excel BHLĐ\\Biểu mẫu ĐBCP định kỳ.xlsx";
